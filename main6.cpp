...
vector<vector<double>> extractConvMat0();

int main_()
{
    VideoCapture* cap = new VideoCapture("/home/pmj-nano/Desktop/1103/130 (3).avi");
    if(!cap->isOpened())
    {
...
//    cout<<hsfatMat[5]<<endl;
//    cout<<"hsfatMat[5].size():   "<<hsfatMat[5].size()<<endl;
    Mat IM_bri_T;
    //
    vector<vector<double>> temp = extractConvMat0();
    vector<Mat> hsfatMat;
    for(int i = 0; i < 15; i++)
    {
        hsfatMat.push_back(Mat(85,85,CV_64FC1,temp[i].data()));
    }
//    Mat tmp = hsfatMat[5];
...
//        cout<<hsfatMat[1]<<endl;
//        cout<<"hsfatMat[1].size():   "<<hsfatMat[1].size()<<endl;
...
//        imshow("IM_bri_T_8U",IM_bri_T_8U);
//        waitKey(0);
